            ...
            <div class="col-md-9 col-lg-10 p-4 bg-dark">
                <div class="p-4 rounded-3 mb-4 d-flex flex-column" style="background-color: #1e2125;">
                    ...
                    <h2 class="fs-5 text-white mb-4">
                        Gerencie o catálogo de serviços prestados na barbearia, preços e descrições.
                    </h2>
                            ...
                            <div class="container-scroll-servicos">
                                <div class="grid-servicos">
                                    @foreach ($servicos as $servico)
                                        ...
                                        <div class="card-servico-custom" data-bs-toggle="modal" data-bs-target="#modalDetalhesServico{{ $servico->id }}">
                                            @if(!empty($servico->foto))
                                                <img src="{{ asset('storage/' . $servico->foto) }}" class="card-servico-img" alt="{{ $servico->nome }}">
                                            @else
                                            ...
                                            <div class="card-servico-corpo">
                                                <h3 class="text-white fs-5 mb-1 montserrat">{{ $servico->nome }}</h3>
                                                
                                                <div class="text-primary small mb-2 fw-semibold">
                                                    <i class="bi bi-cash-coin me-1"></i> Preço: R$ {{ number_format($servico->preco, 2, ',', '.') }}
                                                </div>
                                                
                                                <p class="descricao-cortada text-white-50 small mb-3">
                                                    {{ Str::limit($servico->descricao, 60, '...') }}
                                                </p>
                                                
                                                <div class="mt-auto d-flex justify-content-end align-items-center" onclick="event.stopPropagation();">
                                                    <div>
                                                        <button class="btn btn-sm btn-outline-primary me-1" data-bs-toggle="modal" data-bs-target="#modalEditarServico{{ $servico->id }}" title="Editar">
                                                            <i class="bi bi-pencil"></i>
                                                        </button>
                                                        <button class="btn btn-sm btn-lixeira-dark" data-bs-toggle="modal" data-bs-target="#modalDeletarServico{{ $servico->id }}" title="Deletar">
                                        ...
                                        @include('servicos.modal.details')
                                        @include('servicos.modal.edit')
                                        @include('servicos.modal.delete')
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        @if ($errors->any())
            @if (session()->has('modal_error_id'))
                var modalEditar = new bootstrap.Modal(document.getElementById('modalEditarServico' + '{{ session('modal_error_id') }}'));
                modalEditar.show();
            @else
                var modalCriar = new bootstrap.Modal(document.getElementById('modalServico'));
                modalCriar.show();
            @endif
        @endif
    });
